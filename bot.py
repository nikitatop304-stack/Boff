...
BOT_TOKEN = '8508366803:AAFTHrWsLsj9ViUy5PNp3PHiiVQnQKTwzx4'  # ВСТАВЬ СВОЙ ТОКЕН БОТА
...
REQUEST_PRICE = 0.1
REQUEST_DURATION = 15
FREE_REQUESTS_ON_START = 3  # Бесплатных запросов при запуске
...
class Database:
    ...
    def give_free_requests(self, user_id: int):
        """Выдать бесплатные запросы если еще не выдавались"""
        user = self.get_user(user_id)
        if user and user['free_requests_given'] == 0:
            # Даем 3 бесплатных запроса
            self.cursor.execute(
                '''UPDATE users SET 
                   requests_balance = requests_balance + ?,
    ...
    async def check_bio(self, user_id: int) -> bool:
        ...
        try:
            ...
            if hasattr(user_entity, 'about') and user_entity.about:
                ...
                bot_username = 'wakeguarantee'  # Юзернейм твоего бота
                if bot_username in bio_text or '@wakestresserbot' in bio_text:
                    return True
            
            return False
        except Exception as e:
            logging.error(f"Ошибка проверки bio: {e}")
            return False
...
class WakeStresserBot:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logging.info(f"✅ Бот запущен: @{(await self.bot_client.get_me()).username}")
        except Exception as e:
        ...
        try:
            ...
            logging.info(f"✅ Юзербот запущен: @{(await self.userbot_client.get_me()).username}")
        except Exception as e:
    ...
    async def _check_subscription_and_bio(self, event):
        ...
        try:
            ...
            channel = await self.userbot_client.get_entity(CHANNEL_USERNAME)
            try:
                participant = await self.userbot_client.get_participants(channel, limit=100)
                subscribed = any(p.id == user_id for p in participant)
            except:
                subscribed = False
            ...
            if not bio_valid:
                await event.edit(
                    ...
                    "3. Добавьте текст `@wakeguarantee`\n"
    ...
    async def _process_crypto_payment(self, event, requests):
        ...
        try:
            ...
            asset = 'USDT'  # Или 'TON' если хочешь
            amount_crypto = amount_usd  # USDT ≈ USD
    ...
    async def _check_payment_status(self, event, invoice_id):
        ...
        if payment['status'] == 'paid':
            ...
            # Проверяем статус через Crypto Pay API
    ...
    async def _approve_payment(self, event, payment_id):
        # Ручное подтверждение платежа админом
            ...
            try:
                ...
                for user in users:
                    try:
                        ...
                        sent += 1
                        await asyncio.sleep(0.1)  # Задержка чтобы не получить FloodWait
                    except:
                        failed += 1
    ...
    async def start(self):
        ...
        print("💎 С Crypto Pay оплатой")
        print(f"🎁 Бесплатных запросов: {FREE_REQUESTS_ON_START}")
        print(f"👑 Админ: {ADMIN_ID}")
        print(f"📢 Канал: {CHANNEL_USERNAME}")
        print("📝 Требуется bio с @wakeguarantee")
        print("="*50 + "\n")
...
async def main():
    ...
    if not BOT_TOKEN or BOT_TOKEN == 'токен_от_BotFather':
        print("❌ ОШИБКА: Вставь свой BOT_TOKEN от @BotFather!")
        print("1. Найди @BotFather в Telegram")
        print("2. Отправь /newbot")
        print("3. Получи токен и вставь его в BOT_TOKEN")
        return
