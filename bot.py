...
class SubscriptionChecker:
    ...
    async def check_subscription(self, user_id: int) -> bool:
        """Проверяет подписку на канал через бота"""
        try:
            # Бот может проверять подписку через get_chat_member
            try:
                chat_member = await self.bot_client.get_chat_member(CHANNEL_USERNAME, user_id)
...
class WakeStresserBot:
    ...
    async def initialize(self):
        ...
        self.sub_checker = SubscriptionChecker(self.bot_client)  # Используем бота для проверки подписки
    ...
    async def _register_handlers(self):
        ...
        async def callback_handler(event):
            ...
            if data == 'check_sub':
                await self._check_subscription(event)
    ...
    # ========== ПРОВЕРКА ПОДПИСКИ ==========
    async def _check_subscription(self, event):
        user_id = event.sender_id
            ...
            if not subscribed:
                await event.edit(
                    ...
                    buttons=[
                        ...
                        [Button.inline("🔄 Проверить снова", b"check_sub")]
                    ]
                )
                return
            
            # Подписка подтверждена
            self.db.update_user(user_id, subscribed=True)
